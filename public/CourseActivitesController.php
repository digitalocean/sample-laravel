    ...
    public function coursePictureUpload2(Request $request){
            $this->validate($request, [
               // 'title' => 'required','body' => 'required',
                'coursepic' => 'nullable|max:5999' ]);

              
            }

  $uniqueFileName = uniqid() . $request->get('coursepic')->getClientOriginalName() . '.' . $request->get('coursepic')->getClientOriginalExtension());

                $request->get('coursepic')->move(public_path('files') . $uniqueFileName);
               ...
               // if($request->hasfile('courseimg')){
                    ...
                    //Uploade Image
                    $path = $request->file('coursepic')->storeAs('public/courseimgs', $fileNameToStore);
