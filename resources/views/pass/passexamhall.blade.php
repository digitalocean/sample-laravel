 	...
 	<select  style="height:50px !important;" name="examcode" class="form-control">
		...
		<div class="form-group">
          ...
          <div class="form-group">
					...
					<br/><br/>
<form action="/instruction" method="post" accept-charset="UTF-8">
@csrf
        <input type="hidden" name="examcode" value="{{$exam1code}}">
        @if($exam1code !=0)
        {{$proctor}}
          @if($exam1code != 66)
<input   <?php if($proctor == 0){echo 'disabled';} ?>  style="height:50px !important;"  class="btn btn-success" type="submit" value="Enter Exam Room">
...
<?php
  ...
  $maxref_result = DB::table('ref_result')->where('examcode' , $exam1code)->where('student_id', $student_id)->where('mailing' , 0)->max('id');
  $ref_result = DB::table('ref_result')->where([['id' => $maxref_result]])->first();
  $attempno = $ref_result->id;
?>
    <input type="text" name="examcode" value="{{$exam1code}}">{{$exam1code}}
    <input type="text" name="attempno" value="{{$attempno}}">
    <input <?php if($attempno > 0){}else{echo 'disable';} ?> class="btn btn-success" type="submit" value="Enter Exam Room">{{$attempno}}
