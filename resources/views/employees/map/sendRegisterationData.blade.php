...
<h3 align="center"><b>Invoice</b></h3>
 ...
 <img src="{{ asset('pdf1.png') }}" width ="150px" height="150px" alt="pdf" /> </a> </div>-->
        ...
        <table id="nbc2">
        <thead><tr id="nbc2">
        <td style="text-align:left;width:10%;" >Id</td><td>Subject</td><td   style="text-align:right;width:12%;">Invoice Amount</td>
        </tr></thead><tbody><tr id="nbc2">
        <?php 
        
        $exam=DB::table('exam')->where('examcode',$invoice->subject_id)->first(); ?>
        <td   style="text-align:left;">{{$invoice->id}}</td><td>Regiteration </td><td   style="text-align:right;"> {{$invoice->amount}}$</td>
        </tr>
        </tbody>
